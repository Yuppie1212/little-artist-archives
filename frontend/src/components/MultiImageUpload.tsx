...
interface Props {
  ...
  onChange: (signedIds: string[]) => void
}

const genId = (): string => Math.random().toString(36).slice(2)
const DIRECT_UPLOAD_URL = "/rails/active_storage/direct_uploads"

export default function MultiImageUpload({ existingPhotos, onChange }: Props) {
  const [items, setItems] = useState<ImageItem[]>(
    existingPhotos.map((p) => ({
      id: genId(),
      signedId: p.signed_id,
      previewUrl: p.url,
      uploading: false,
      progress: 100,
    }))
  ...
  useEffect(() => {
    const signedIds = items
      .filter((item) => !item.uploading && item.signedId)
            ...
            {item.uploading && (
              <div className="multi-upload-progress">
                <div
                  ...
                  style={{ "--w": `${item.progress}%` } as React.CSSProperties}
                />
